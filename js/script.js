window.onload = function () {
  ...
  // 인기물품 화면 출력
  let POPULAR_GOOD;
  let popularshow = 1; // 목록 중에 0번(=populargood-1)을 보여준다
  ...
  function showPopularIconGood() {
    ...
    const seIcon = new Swiper(".sw-icon", {
      slidesPerView: 7, // 7개씩 보여줌
      slidesPerGroup: 7, // 7개씩 묶어서 보여줌
      spaceBetween: 10, // 간격을 10으로 함
      navigation: {
    ...
    const tags = document.querySelectorAll(".popular-slide a");
    tags.forEach(function (item, index) {
      ...
      // 클릭하면 버튼(.popular-more)의 글자를
      // 클릭된 타이틀의 글자로 변경한다.
      item.addEventListener("click", function (e) {
        // a 태그이므로 href가 자동으로 적용된다.
        // 웹 브라우저에 갱신이되므로 UI를 위해 막아야 한다.
        e.preventDefault();
  ...
  toggleListArr.forEach(function (item) {
    item.addEventListener("click", function (event) {
      // 이벤트를 했을 때 다른 이벤트로 넘어가지 않도록 막아주는 것
  ...
  allMenu.addEventListener("mouseleave", function () {
    allMenu.classList.remove("active");
  });
